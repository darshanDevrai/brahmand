...
use crate::query_planner::{logical_plan::logical_plan::{LogicalPlan, Projection}, optimizer::{errors::{OptimizerError, Pass}, optimizer_pass::{OptimizerPass, OptimizerResult}}, plan_ctx::plan_ctx::PlanCtx, transformed::Transformed};
    ...
    fn optimize(&self, logical_plan: Arc<LogicalPlan>, plan_ctx: &mut PlanCtx) -> OptimizerResult<Transformed<Arc<LogicalPlan>>> {
        let transformed_plan = match logical_plan.as_ref() {
            ...
            LogicalPlan::Scan(scan) => {
                let table_ctx_opt = plan_ctx.get_mut_table_ctx_opt_from_alias_opt(&scan.table_alias).map_err(|e| OptimizerError::PlanCtx { pass: Pass::ProjectionPushDown, source: e})?;
