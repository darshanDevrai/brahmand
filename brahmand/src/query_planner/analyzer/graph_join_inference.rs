...
use crate::{ graph_catalog::graph_schema::GraphSchema, query_planner::{analyzer::{analyzer_pass::{AnalyzerPass, AnalyzerResult}, errors::Pass, graph_context::{self, GraphContext}}, logical_expr::logical_expr::{Column, Direction, LogicalExpr, Operator, OperatorApplication, PropertyAccess, TableAlias}, logical_plan::logical_plan::{GraphJoins, GraphRel, Join, JoinType, LogicalPlan}, plan_ctx::plan_ctx::PlanCtx, transformed::Transformed}};
...
impl GraphJoinInference {
    ...
    fn infer_graph_join(&self, graph_rel: &GraphRel, plan_ctx: &mut PlanCtx, graph_schema: &GraphSchema, collected_graph_joins: &mut Vec<Join>, joined_entities: &mut HashSet<String>) -> AnalyzerResult<()> {
        ...
        let graph_context = graph_context::get_graph_context(graph_rel, plan_ctx, graph_schema, Pass::GraphJoinInference)?;
        ...
        if graph_context.rel.table_ctx.should_use_edge_list() {
            ...
            self.handle_edge_list_traversal(graph_rel, graph_context, left_node_id_column, right_node_id_column, is_standalone_rel, collected_graph_joins, joined_entities)

        } else {
            
            self.handle_bitmap_traversal(graph_context, left_node_id_column, right_node_id_column, is_standalone_rel, collected_graph_joins, joined_entities)
        }
    ...
    fn handle_edge_list_traversal(&self, graph_rel: &GraphRel, graph_context: GraphContext, left_node_id_column: String, right_node_id_column: String, is_standalone_rel: bool, collected_graph_joins: &mut Vec<Join>, joined_entities: &mut HashSet<String>, ) -> AnalyzerResult<()>{
    ...
    fn handle_bitmap_traversal(&self, graph_context: GraphContext, left_node_id_column: String, right_node_id_column: String, is_standalone_rel: bool, collected_graph_joins: &mut Vec<Join>, joined_entities: &mut HashSet<String>, ) -> AnalyzerResult<()>{
...
mod tests {
    ...
    use crate::{
        ...
        query_planner::{
            logical_expr::logical_expr::{Column, Direction, LogicalExpr, Operator, PropertyAccess, TableAlias},
            logical_plan::logical_plan::{GraphNode, GraphRel, LogicalPlan, Scan, JoinType, Projection, ProjectionItem},
    ...
    fn create_test_graph_schema() -> GraphSchema {
        ...
        nodes.insert("Person".to_string(), NodeSchema {
        ...
        nodes.insert("Company".to_string(), NodeSchema {
        ...
        relationships.insert("FOLLOWS".to_string(), RelationshipSchema {
            ...
            column_names: vec!["from_id".to_string(), "to_id".to_string(), "since".to_string()],
        ...
        relationships.insert("WORKS_AT".to_string(), RelationshipSchema {
            table_name: "WORKS_AT".to_string(),
            column_names: vec!["from_id".to_string(), "to_id".to_string(), "position".to_string()],
            from_node: "Person".to_string(),
    ...
    fn setup_plan_ctx_with_graph_entities() -> PlanCtx {
        ...
        plan_ctx.insert_table_ctx(
            "p1".to_string(),
            TableCtx::build("p1".to_string(), Some("Person".to_string()), vec![], false, true),
        );
        plan_ctx.insert_table_ctx(
            "p2".to_string(),
            TableCtx::build("p2".to_string(), Some("Person".to_string()), vec![], false, true),
        );
        plan_ctx.insert_table_ctx(
            "p3".to_string(),
            TableCtx::build("p3".to_string(), Some("Person".to_string()), vec![], false, true),
        );

        // Add company node
        plan_ctx.insert_table_ctx(
            "c1".to_string(),
            TableCtx::build("c1".to_string(), Some("Company".to_string()), vec![], false, true),
        );
        
        // Add follows relationships
        plan_ctx.insert_table_ctx(
            "f1".to_string(),
            TableCtx::build("f1".to_string(), Some("FOLLOWS".to_string()), vec![], true, true),
        );
        plan_ctx.insert_table_ctx(
            "f2".to_string(),
            TableCtx::build("f2".to_string(), Some("FOLLOWS".to_string()), vec![], true, true),
        );

        // Add works_at relationship
        plan_ctx.insert_table_ctx(
            "w1".to_string(),
            TableCtx::build("w1".to_string(), Some("WORKS_AT".to_string()), vec![], true, true),
        );
    ...
    fn test_no_graph_joins_when_no_graph_rels() {
        ...
        let result = analyzer.analyze_with_graph_schema(graph_node.clone(), &mut plan_ctx, &graph_schema).unwrap();
    ...
    fn test_edge_list_same_node_type_outgoing_direction() {
        ...
        plan_ctx.get_mut_table_ctx("f1").unwrap().set_use_edge_list(true);
        ...
        let input_logical_plan = Arc::new(LogicalPlan::Projection(Projection {
            ...
            items: vec![
                ProjectionItem {
                    expression: LogicalExpr::PropertyAccessExp(PropertyAccess { table_alias: TableAlias("p1".to_string()), column: Column("name".to_string()) }),
                    col_alias: None,
                }
            ],
        }));
        ...
        let result = analyzer.analyze_with_graph_schema(input_logical_plan, &mut plan_ctx, &graph_schema).unwrap();
            ...
            Transformed::Yes(plan) => {
                match plan.as_ref() {
                    LogicalPlan::GraphJoins(graph_joins) => {
                        ...
                        assert!(matches!(graph_joins.input.as_ref(), LogicalPlan::Projection(_)));
                        ...
                        match (&rel_join_condition.operands[0], &rel_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(rel_prop), LogicalExpr::PropertyAccessExp(left_prop)) => {
                        ...
                        // Check operands are PropertyAccessExp with correct table aliases and columns
                        match (&right_join_condition.operands[0], &right_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(left_prop), LogicalExpr::PropertyAccessExp(rel_prop)) => {
    ...
    fn test_edge_list_different_node_types() {
        ...
        // Set the relationship to use edge list
        plan_ctx.get_mut_table_ctx("w1").unwrap().set_use_edge_list(true);
            ...
            Transformed::Yes(plan) => {
                match plan.as_ref() {
                    LogicalPlan::GraphJoins(graph_joins) => {
                        ...
                        // Check operands are PropertyAccessExp with correct table aliases and columns
                        match (&right_join_condition.operands[0], &right_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(right_prop), LogicalExpr::PropertyAccessExp(rel_prop)) => {
    ...
    fn test_bitmap_traversal() {
        ...
        let f1_scan = create_scan_plan("f1", "FOLLOWS");
         ...
         plan_ctx.insert_table_ctx(
            "f1".to_string(),
            TableCtx::build("f1".to_string(), Some("FOLLOWS_outgoing".to_string()), vec![], true, true),
        );
    ...
    fn test_standalone_relationship_edge_list() {
        ...
        // Set the relationship to use edge list
        plan_ctx.get_mut_table_ctx("f2").unwrap().set_use_edge_list(true);
            ...
            Transformed::Yes(plan) => {
                match plan.as_ref() {
                    LogicalPlan::GraphJoins(graph_joins) => {
                        ...
                        match (&first_join_condition.operands[0], &first_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(rel_prop), LogicalExpr::PropertyAccessExp(left_prop)) => {
                        ...
                        match (&second_join_condition.operands[0], &second_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(rel_prop), LogicalExpr::PropertyAccessExp(right_prop)) => {
                                assert_eq!(rel_prop.table_alias.0, "f2");
                                assert_eq!(rel_prop.column.0, "from_id");
            ...
            Transformed::Yes(plan) => {
                match plan.as_ref() {
                    LogicalPlan::GraphJoins(graph_joins) => {
                        ...
                        match (&right_join_condition.operands[0], &right_join_condition.operands[1]) {
                            (LogicalExpr::PropertyAccessExp(left_prop), LogicalExpr::PropertyAccessExp(right_prop)) => {
                                assert_eq!(left_prop.table_alias.0, "p1");
                                assert_eq!(left_prop.column.0, "id");
            ...
            Transformed::Yes(plan) => {
                match plan.as_ref() {
                    LogicalPlan::GraphJoins(graph_joins) => {
                        ...
                        let rel_aliases: Vec<&String> = graph_joins.joins.iter()
                            .map(|j| &j.table_alias)
                            .collect();
                        
                        // Should contain joins for both relationships
                        assert!(rel_aliases.iter().any(|&alias| alias == "f1" || alias == "w1"));
                        ...
                        let join_aliases: Vec<&String> = graph_joins.joins.iter().map(|j| &j.table_alias).collect();
                            ...
                            match join.table_alias.as_str() {
                                "w1" => {
                                    ...
                                    match (&join_condition.operands[0], &join_condition.operands[1]) {
                                        ...
                                        _ => panic!("Expected PropertyAccessExp operands for w1 join"),
                                    }
                                },
                                "p2" => {
                                ...
                                "f1" => {
                                    ...
                                    match (&join_condition.operands[0], &join_condition.operands[1]) {
                                        ...
                                        _ => panic!("Expected PropertyAccessExp operands for f1 join"),
                                    }
                                },
                                "p1" => {
