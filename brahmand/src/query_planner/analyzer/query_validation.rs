    ...
    fn analyze_with_graph_schema(&self, logical_plan: Arc<LogicalPlan>, plan_ctx: &mut PlanCtx, graph_schema: &GraphSchema) -> AnalyzerResult<Transformed<Arc<LogicalPlan>>> {
        let transformed_plan = match logical_plan.as_ref() {
            ...
            LogicalPlan::GraphRel(graph_rel) => {
                ...
                let left_ctx = plan_ctx.get_node_table_ctx(left_alias).map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;
                let right_ctx = plan_ctx.get_node_table_ctx(right_alias).map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;

                let left_label = left_ctx.get_label_str().map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;
                let right_label = right_ctx.get_label_str().map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;
                ...
                let rel_ctx = plan_ctx.get_mut_table_ctx(&graph_rel.alias).map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;

                let rel_lable = rel_ctx.get_label_str().map_err(|e| AnalyzerError::PlanCtx { pass: Pass::QueryValidation, source: e})?;
                ...
                if rel_schema.from_node == *from && rel_schema.to_node == *to {
                    ...
                    // return error
                    Err(AnalyzerError::InvalidRelationInQuery { rel: graph_rel.alias.clone() })?
                }
